...
const DecisionRow = memo(function DecisionRow({ decision, onRowClick }: DecisionRowProps) {
  ...
  return (
    ...
    >
      <td className="px-6 py-4">
        <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium border ${style.badge}`}>
          {style.icon && <span className="mr-1.5">{style.icon}</span>}
          {decision.decision.toUpperCase()}
        </div>
      </td>
      <td className="px-6 py-4">
        <div className="text-sm font-semibold text-gray-900">{formatAmount(decision.amount)}</div>
      </td>
      <td className="px-6 py-4">
        <div className="text-sm text-gray-900 font-medium">{decision.payee}</div>
      </td>
      <td className="px-6 py-4">
      ...
      <td className="px-6 py-4">
        <div className="flex items-center">
          <div className={`w-2 h-2 rounded-full mr-2 ${
            decision.latency < 200 ? 'bg-green-400' : 
            decision.latency < 400 ? 'bg-yellow-400' : 'bg-red-400'
          }`}></div>
          <span className="text-sm text-gray-600">{decision.latency}ms</span>
        </div>
      </td>
      <td className="px-6 py-4">
        <span className="text-sm text-gray-500">{formatTimestamp(decision.timestamp, navigator.language, { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
      </td>
      <td className="px-6 py-4">
